...
int sendEventDataToHonoInstance (char * host_name, int port, char * tenant_name, char * device_id, char * field, char * event)
{
	FILE *fp;
	int code;
	int status;
...
void sendTeleDataToHono(struct afb_req req){
	...
	if(status == 1){
		afb_req_success(req, NULL, "Telemetry data sent and accepted");
	}
	else{
		switch(status_code){
			case 408:
...
void sendEventDataToHono(struct afb_req req){
	...
	const char *portData = afb_req_value(req, "port");
	const char *field_data = afb_req_value(req, "field");
	const char *eventData = afb_req_value(req, "event");
	const char *tenantName = afb_req_value (req, "tenant_name");
	port_data = atoi(portData);
	...
	int status = sendEventDataToHonoInstance(hostName, port_data, tenantName, deviceID, field_data, eventData);

	if(status == 1){
		afb_req_success(req, NULL, "Event data sent and accepted");
	}
	else{
		switch(status_code){
			case 408:
...
int init()
{
	AFB_NOTICE("Cloud API initS");
	return 0;
}

int preinit()
{
	AFB_NOTICE("Cloud API preinit");
	return 0;
}

const afb_verb_v2 verbs[] = {
	{.verb="connect", .callback=registerDeviceToHono, .info = "Connects device to hono"},
	{.verb="sendt", .callback=sendTeleDataToHono, .info = "Send Telemetry data to hono"},
	{.verb="sende", .callback=sendEventDataToHono, .info = "Send Event data to hono"},
	{.verb=NULL }
};
